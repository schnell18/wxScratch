...
class TestModel(dv.DataViewIndexListModel):
    ...
    def Compare(self, item1, item2, col, ascending):
        ...
        if col == 0:
            return cmp(int(self.data[row1][col]),
int(self.data[row2][col]))
        else:
    ...
    def DeleteRows(self, rows):
        ...
        rows = list(rows)
        # use reverse order so the indexes don't change as we remove
        # items
        ...
        for row in rows:
            ...
            del self.data[row]
            # notify the view(s) using this model that it has been
            # removed
            self.RowDeleted(row)
...
self.OnValueChanged, self.dvc)
    ...
    def OnNewView(self, evt):
        f = wx.Frame(None, title="New view, shared model",
size=(600,400))
    ...
    def OnDeleteRows(self, evt):
        ...
        # care
        # of notifying the view (and any other observers) that the
        # change has
        # happened.
